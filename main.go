...
var (
	// Version bot.
	Version string
	// Revision or Commit this binary.
	Revision string
	// BuildDate this binary.
	BuildDate string
	// GoVersion running this binary.
	GoVersion = runtime.Version()
	// StartTime has the time.
	StartTime = time.Now()
)
...
func main() {
	...
	a := kingpin.New("prom-bot", "Bot for Prometheus Alertmanager")
	a.HelpFlag.Short('h')

	a.Flag("alertmanager.url", "The URL used to connect the alertmanager").
		Envar("ALERTMANAGER_URL").
		Default("http://localhost:9093/").
		URLVar(&config.alertmanager)

	a.Flag("bolt.path", "The path to bolt persists its data").
		Envar("BOLT_PATH").
		Default("/tmp/bot.db").
		StringVar(&config.boltPath)

	a.Flag("consul.url", "The URL used to connect the consul store").
		Envar("CONSUL_URL").
		Default("localhost:8500").
		URLVar(&config.consul)

	a.Flag("listen.addr", "The address bot listens for incoming webhooks").
		Envar("LISTEN_ADDR").
		Default("0.0.0.0:8080").
		StringVar(&config.listenAddr)

	a.Flag("log.json", "Tell the application to log json & not key value pairs").
		Envar("LOG_JSON").
	...
	a.Flag("telegram.admin", "The ID of Telegram Admin").
		Required().
		Envar("TELEGRAM_ADMIN").
		IntsVar(&config.telegramAdmins)

	a.Flag("telegram.token", "The token used to connect Telegram").
		Required().
